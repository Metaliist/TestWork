import { appendFile, appendFileSync, WriteFileOptions, readFile, readFileSync, open, access, mkdirSync, writeFile } from 'fs';
                    ...
                    request.post(zap, function (error, body) {
                        if (error) {
                            ...
                            for (let i = 0; i < body.body.length; i++) {
                                s = s.replace(body.body[i].s[body.body[i].s.length - 1],body.body[i].s[0]);
                            }
                            console.log(s);
                            if (s.length != 0) {
